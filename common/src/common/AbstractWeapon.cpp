...
AbstractWeapon::AbstractWeapon(const sf::Vector2f& size,
        ...
        AbstractDrawableObject({}, size,
                               ...
                               name.empty() ? 1 : utils::j3x::getInt(RM.getObjectParams("weapons", name), "frames_number"),
                               name.empty() ? 0.0f :utils::j3x::getFloat(RM.getObjectParams("weapons", name), "frame_duration"))
{
}
