        ...
        time_elapsed_(0.0f)
{
    ...
    base_pos_ = father_->getPosition() +
                sf::Vector2f{0.0f, -father_->getSize().y / 2.0f + CONF<float>("graphics/tutorial_arrow_offset")};
...
bool TutorialArrow::update(float time_elapsed)
{
    ...
    arrow_.setPosition(base_pos_ +
                       sf::Vector2f(0.0f, std::sin(time_elapsed_ * M_PI * 2.0f) *
                                          CONF<float>("graphics/tutorial_arrow_amplitude")));
