...
HealthBar::HealthBar(const sf::Vector2f& position) :
        ...
        health_text_("Health: 100%", RM.getFont(),
                     16 * CFG.getFloat("graphics/user_interface_zoom"))
{
    health_text_.setFillColor(sf::Color(CFG.getInt("graphics/font_color")));
    health_text_.setPosition(
            this->getPosition() - sf::Vector2f{HEALTH_SIZE_X_ / 2.0f * CFG.getFloat("graphics/user_interface_zoom"),
                                               30.0f * CFG.getFloat("graphics/user_interface_zoom")});
}
...
void HealthBar::updateHealth(int health)
{
    ...
    shape_.setSize({HEALTH_SIZE_X_ * percentage / 100.0f * CFG.getFloat("graphics/user_interface_zoom"),
                    HEALTH_SIZE_Y_ * CFG.getFloat("graphics/user_interface_zoom")});
