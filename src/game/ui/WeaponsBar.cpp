...
WeaponsBar::WeaponsBar(const sf::Vector2f& position) :
        current_highlight_(position,
                           ...
                           &AbstractResourceManager::getInstance().getTexture("current_weapon_highlight")),
        AbstractDrawableObject(position,
                               {SIZE_X_ * CFG.getFloat("user_interface_zoom"),
                                SIZE_Y_ * CFG.getFloat("user_interface_zoom")},
                               &AbstractResourceManager::getInstance().getTexture("weapons_bar"))
{
    for (int i = 0; i < SLOTS_; ++i)
    {
        ammo_.emplace_back("0", AbstractResourceManager::getInstance().getFont(), 16 * CFG.getFloat("user_interface_zoom"));
    ...
    for (const auto& weapon : weapons)
    {
        ...
        if (weapon_cast != nullptr)
        {
            ...
            weapons_.push_back({weapon_pos, {WEAPON_SIZE_X_ * CFG.getFloat("user_interface_zoom"),
                                             WEAPON_SIZE_Y_ * CFG.getFloat("user_interface_zoom")},
                                &AbstractResourceManager::getInstance().getTexture("weapon_mini_" + weapon_cast->getName())});
